...
**Parent-slot matching follows e-mail changes through the tracked parent source.** A Rondo parent relationship imported from Sportlink is authorized by its `rondo_club_parents` mapping for that exact child, even when it has no UI-created sync status yet. Keep the mapped source e-mail in the queued desired payload so a changed Rondo parent e-mail can still find and update the child's existing Sportlink slot. Match the desired e-mail first, then exactly one mapped source e-mail. When the e-mail is missing, a partially filled slot is safe only if it is the sole candidate and every existing name, e-mail, and phone value agrees after name and Dutch phone normalization. Conflicting or ambiguous slots stay blocked. Verify saved values with the same normalization because Sportlink reformats names and phone numbers. Notify Rondo's profile-change audit only after every child job for that parent is resolved; callback failure must not turn an already completed Sportlink write back into a retry.
