...
**Source-owned empty values must be sent explicitly, but only when the source field was present.** Omitting a canonical field from a PUT preserves its existing Rondo Club value. For Sportlink-owned fields that can legitimately become empty, include `null` when a complete source response explicitly contains an empty value. `preparePerson()` therefore writes `spelactiviteit: null` for `KernelGameActivities: ''`, but omits `spelactiviteit` when the property itself is absent. The individual `/general` response is partial and omits `KernelGameActivities` and `AgeClassDescription`; overlay it on the latest complete SearchMembers snapshot before preparing the person, and never treat an omitted partial-response property as an empty source value.
