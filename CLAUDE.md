...
**Source-owned empty values must be sent explicitly, but only when the source field was present.** Omitting a canonical field from a PUT preserves its existing Rondo Club value. For Sportlink-owned fields that can legitimately become empty, include `null` when a complete source response explicitly contains an empty value. `preparePerson()` therefore writes `spelactiviteit: null` for `KernelGameActivities: ''`, normalizes repeated whitespace such as `Veld -  Algemeen`, but omits `spelactiviteit` when the property itself is absent. The individual `/general` response is partial and omits `KernelGameActivities` and `AgeClassDescription`. With `--fetch`, obtain a fresh exact-ID SearchMembers result first, validate both fields are present, and overlay `/general` on that result. An absent active match is retried with the inactive status filter. Failed, ambiguous, or incomplete fresh searches must stop before person updates; never fall back to a stale activity or treat a missing property as empty. Runs without `--fetch` use the stored bulk snapshot.
...
**Reverse-sync date verification must normalize Sportlink's Dutch display value.** Sportlink saves an ISO `datum_vog` value but renders it back in edit mode as a localized date such as `4 sept 2026`. Keep both `sep` and Sportlink's four-letter `sept` in `dutchDateToISO()`; a literal comparison makes a successful write look failed and retries the same queued change every five minutes.

**Existing parent slots change only from an explicit Rondo e-mail audit event.** A mismatch between a Sportlink parent slot and Rondo `email_1` is not evidence of drift: the slot may intentionally use `email_2`. For `email_primary`, `email_secondary`, `email_promoted`, or `email_removed`, queue the exact audited old-to-new replacement for every current child relationship. The writer must find exactly one slot with the old address and a compatible parent name, change only that slot's e-mail input, preserve its name and phone, and verify all three values after saving. Historical mappings or generic person modifications must never authorize this overwrite. New UI-created parent relationships remain a separate full-slot job. Notify Rondo's e-mail audit fields only after every child job for that parent is resolved; callback failure must not turn an already completed Sportlink write back into a retry.
