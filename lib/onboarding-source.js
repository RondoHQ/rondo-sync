...
async function checkSource({ candidate, member, steps, request, now = () => new Date() }) {
  ...
  if (state === 'unknown') {
    // Parked, rejected, aspirant and transfer records are valid Sportlink
    // observations, but they are not eligible for onboarding. Keep them
    // incomplete without turning every People run into a partial failure or
    // spending browser requests on source data that cannot be applied yet.
    deferred.push({ part: 'person', message: 'Sportlink registration status not confirmed' });
  } else {
  ...
  return { complete: result.complete === true, coverage, errors, deferred };
}
