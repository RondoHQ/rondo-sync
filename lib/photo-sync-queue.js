...
const { logPhotoEvent } = require('./photo-sync-log');
...
async function runPhotoQueue({ api, sportlink, knvbId = null, clock = () => new Date(), limit = 25 }) {
  ...
  for (const job of jobs) {
    ...
    try {
      ...
      result.results.push({ personId: job.person_id, knvbId: job.knvb_id, source: 'rondo', destination: 'sportlink', status: outcome.state === 'synced' ? 'changed' : 'skipped', ...outcome });
    } catch (error) {
      result.failed++;
      result.success = false;
      // A post-claim failure remains sending/review in WordPress and cannot be retried here.
      result.results.push({ personId: job.person_id, knvbId: job.knvb_id, source: 'rondo', destination: 'sportlink', status: 'failed', reason: error.message, state: 'error', error: error.message });
    }
  }
  return result;
}
...
async function runPhotoSync(options = {}) {
  ...
  try {
    ...
    options.logger?.log(`Photos from Rondo to Sportlink/voetbal.nl: ${result.synced} changed, ${result.failed} failed`);
    for (const item of result.results) logPhotoEvent(options.logger, item);
    return result;
