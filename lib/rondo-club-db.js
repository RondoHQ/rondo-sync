...
function initDb(db) {
  ...
  const seedMappings = db.prepare(`
    INSERT OR IGNORE INTO free_field_mappings (source_field, target_field, value_type, updated_at)
    VALUES (?, ?, ?, ?)
  `);
  for (const source of FREE_FIELD_SOURCES) {
    ...
    seedMappings.run(source, target, type, now);
  }
 ...
 * @param {Object} db - Database connection
 * @returns {Array<{source_field: string, target_field: string|null, value_type: string}>}
 */
 ...
 * @param {Object} db - Database connection
 * @param {Array<{source_field: string, target_field: string|null, value_type: string}>} mappings
 */
function upsertFreeFieldMappings(db, mappings) {
  const now = new Date().toISOString();
  const stmt = db.prepare(`
    INSERT INTO free_field_mappings (source_field, target_field, value_type, updated_at)
    VALUES (@source_field, @target_field, @value_type, @updated_at)
