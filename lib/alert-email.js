...
const PIPELINE_CONFIG = {
  ...
  reverse: { displayName: 'Reverse sync' },
...
async function sendFailureAlert({ pipeline, runId, error, startedAt }) {
  ...
  if (!process.env.LETTERMINT_API_TOKEN) {
    console.warn('[alert-email] LETTERMINT_API_TOKEN not set, skipping failure alert');
    return;
  }
  if (!process.env.LETTERMINT_FROM_EMAIL || !process.env.OPERATOR_EMAIL) {
    console.warn('[alert-email] LETTERMINT_FROM_EMAIL or OPERATOR_EMAIL not set, skipping failure alert');
    return;
  }
...
/** Send one operator request for a reverse-sync item that needs manual repair. */
async function sendActionRequiredAlert({ knvbId, fields, reason }) {
  if (!process.env.LETTERMINT_API_TOKEN || !process.env.LETTERMINT_FROM_EMAIL || !process.env.OPERATOR_EMAIL) {
    console.warn('[alert-email] Action-required mail settings are incomplete, skipping alert');
    return;
  }

  const dashboardUrl = process.env.DASHBOARD_URL || 'http://localhost:3000';
  const fieldList = Array.from(new Set(fields || [])).join(', ');
  const subject = `[Rondo Sync] ACTION NEEDED: reverse sync ${knvbId}`;
  const safeReason = escapeHtml(reason || 'Sportlink vraagt om handmatig herstel.');
  const safeKnvbId = escapeHtml(knvbId);
  const safeFields = escapeHtml(fieldList);
  const html = `<p>De reverse sync is voor <strong>${safeKnvbId}</strong> geparkeerd.</p>
<p><strong>Velden:</strong> ${safeFields}</p>
<p><strong>Te herstellen:</strong> ${safeReason}</p>
<p>Na herstel probeert de sync dit dagelijks opnieuw. De vijfminutenrun blijft intussen groen.</p>
<p><a href="${escapeHtml(dashboardUrl)}">Open het syncdashboard</a></p>`;
  const text = `ACTIE NODIG: REVERSE SYNC\n\nLid: ${knvbId}\nVelden: ${fieldList}\nTe herstellen: ${reason}\n\nNa herstel probeert de sync dit dagelijks opnieuw. De vijfminutenrun blijft intussen groen.\n\n${dashboardUrl}`;
  ...
  await lettermint.email
    ...
    .text(text)
    .send();
  console.log(`[alert-email] Action-required alert sent for reverse sync ${knvbId}`);
}
...
async function sendOverdueAlert(overduePipelines) {
  // Validate environment
  if (!process.env.LETTERMINT_API_TOKEN) {
    console.warn('[alert-email] LETTERMINT_API_TOKEN not set, skipping overdue alert');
    return;
  }
  if (!process.env.LETTERMINT_FROM_EMAIL || !process.env.OPERATOR_EMAIL) {
    console.warn('[alert-email] LETTERMINT_FROM_EMAIL or OPERATOR_EMAIL not set, skipping overdue alert');
    return;
  }
...
if (require.main === module) {
  ...
  if (command === 'send-failure-alert') {
    ...
    console.error('  LETTERMINT_API_TOKEN  - Lettermint API Token');
    console.error('  LETTERMINT_FROM_EMAIL - Verified sender email address');
