...
function closedSeasonEnd(row, today) {
  ...
  // SeasonDescription can identify the season in which a continuing role
  // started. An explicitly active, open relation must not get an invented end.
  if (String(row.Status || '').trim().toUpperCase() === 'ACTIVE') return null;
...
/** Sportlink can mark an undated membership inactive, especially club teams. */
function isTeamMembershipCurrent(row) {
  return String(row.Status || '').trim().toUpperCase() !== 'INACTIVE' && !row.RelationEnd;
}

module.exports = { normalizeTeamMembershipSeasons, isTeamMembershipCurrent };
