...
function buildSponsitLapostaPlan(records, rondoPeople = []) {
  ...
  for (const [email, grouped] of byEmail) {
    ...
    const businessclub = candidate.sponsorFields.sponsor_pass_variant === 'businessclub';
    const personName = [candidate.createFields.first_name, candidate.createFields.last_name]
      .filter(Boolean)
      .join(' ')
      .trim();
    members.push({
      ...
      email,
      custom_fields: {
        voornaam: candidate.createFields.first_name,
        achternaam: candidate.createFields.last_name,
        businessclub: businessclub ? 'Ja' : 'Nee',
        bedrijfsnaam: candidate.createFields.company_name || personName,
