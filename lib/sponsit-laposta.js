...
const { familyName, personNameKey } = require('./person-name');
...
function buildSponsitLapostaPlan(records, rondoPeople = [], rondoSponsors = []) {
  ...
  const entries = [
    ...companies.flatMap((company) => company.people.map((person) => ({ company, person }))),
    ...buildManualBusinessclubEntries(rondoPeople, rondoSponsors)
  ];
  ...
  for (const [email, grouped] of byEmail) {
    ...
    const identities = new Set(grouped.map((entry) => personNameKey(entry.person.fields)));
    if (identities.size > 1) {
      grouped.forEach((entry) => quarantined.push({ candidate: entry.person, reason: 'duplicate_sponsit_email' }));
      continue;
    }

    const entry = grouped.find((item) => item.manualBusinessclub)
      || grouped.find((item) => item.company.fields.sponsor_role === 'businessclub')
      || grouped[0];
    const { company, person } = entry;
    const resolution = entry.rondoPerson
      ? { person: entry.rondoPerson }
      : rondoPlan.resolvedPeople.get(person.sourceKey);
    ...
    const sponsitEntry = grouped.find((item) => item.company.sponsitContactId || item.person.sponsitPersonId);
    members.push({
      sourceKey: person.sourceKey,
      email,
      custom_fields: {
        voornaam: person.fields.first_name,
        achternaam: familyName(person.fields),
        businessclub: businessclub ? 'Ja' : 'Nee',
        bedrijfsnaam: company.title,
        sponsorvariant: businessclub ? 'Businessclub AWC' : 'AWC Sponsor',
        sponsitcontactid: sponsitEntry?.company.sponsitContactId ? String(sponsitEntry.company.sponsitContactId) : '',
        sponsitpersoonid: sponsitEntry?.person.sponsitPersonId ? String(sponsitEntry.person.sponsitPersonId) : '',
...
/** Include active, manually managed Businessclub contacts that have no Sponsit company ID. */
function buildManualBusinessclubEntries(rondoPeople, rondoSponsors) {
  const peopleById = new Map(rondoPeople.map((person) => [Number(person.id), person]));
  const entries = [];
  for (const sponsor of rondoSponsors) {
    const fields = sponsor.fields || {};
    if (sponsor.status !== 'publish'
      || fields.sponsor_role !== 'businessclub'
      || String(fields.sponsit_contact_id || '').trim()) continue;

    for (const contact of fields.contacts || []) {
      const rondoPerson = peopleById.get(Number(contact.person_id));
      if (!rondoPerson) continue;
      entries.push({
        company: {
          title: sponsor.title,
          sponsitContactId: '',
        ...
        person: {
          sourceKey: `rondo-sponsor:${sponsor.id}:person:${rondoPerson.id}`,
          sponsitPersonId: '',
          fields: rondoPerson.fields || {}
        },
        rondoPerson,
        manualBusinessclub: true
      });
    }
  }
  return entries;
