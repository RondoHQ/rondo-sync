 ...
 * Rondo Club stores names, Sportlink uses codes.
 */
...
const SPORTLINK_FIELD_MAP = {
  ...
  'datum_vog': { page: 'other', selector: 'input[name="Remarks8"]', type: 'text' },
  ...
  'country': { page: 'address', selector: 'select[name="CountryCode"]', type: 'select' }
};
...
async function enterEditMode(page, pageType = 'general', expectedSelector = null) {
  ...
  // Current Sportlink UI uses multiple "Wijzig" buttons on /general.
  // Button indices: 0=member type, 1=personal, 2=communication, 3=address, 4=parental, 5=nationality
  if (pageType === 'general') {
...
async function syncSinglePage(page, knvbId, pageType, pageChanges, credentials, options = {}) {
  ...
  for (const change of pageChanges) {
    ...
    // Convert country name to country code for Sportlink select
    if (change.field_name === 'country' && change.new_value) {
      const code = COUNTRY_NAME_TO_CODE[change.new_value.toLowerCase()];
      if (code) {
        change.new_value = code;
      } else {
        // If value is already a 2-letter code, keep it; otherwise log warning
        if (change.new_value.length !== 2) {
          logger?.error(`Unknown country name "${change.new_value}", cannot convert to code`);
        }
      }
    }
  }
