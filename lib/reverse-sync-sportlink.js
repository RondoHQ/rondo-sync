...
async function syncSinglePage(page, knvbId, pageType, pageChanges, credentials, options = {}) {
  ...
  // Detect former members — Sportlink shows "Oud bondslid" with no editable sections
  const bodyText = await page.locator('body').textContent().catch(() => '');
  if (bodyText.includes('Oud bondslid')) {
    throw new Error(`Member ${knvbId} is "Oud bondslid" (former member) — not editable in Sportlink`);
  }
...
async function runReverseSyncMultiPage(options = {}) {
  ...
  try {
    ...
    for (const [knvbId, pageChanges] of changesByMemberAndPage) {
      ...
      if (result.success) {
      ...
      } else if (/Oud bondslid/i.test(result.error || '')) {
        // Former member — not editable in Sportlink, mark changes as synced to stop retrying
        const allFieldNames = [
          ...pageChanges.general.map(c => c.field_name),
          ...pageChanges.address.map(c => c.field_name),
          ...pageChanges.other.map(c => c.field_name),
        ...
        markChangesSynced(db, knvbId, allFieldNames);
        logger?.log(`Skipping ${knvbId}: former member (Oud bondslid), marked ${fieldCount} change(s) as synced`);
        synced++;
      } else {
