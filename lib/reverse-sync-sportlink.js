...
const { openDb, getUnsyncedContactChanges, markChangesSynced, markChangesActionRequired, supersedePendingChanges, updateSportlinkTimestamps, getUnsyncedChanges } = require('./rondo-club-db');
...
const { extractFieldValue } = require('./detect-rondo-club-changes');
const { sendActionRequiredAlert } = require('./alert-email');
...
const SAVE_BUTTON_WAIT_SELECTOR = SAVE_BUTTON_SELECTORS.join(', ');
const SAVE_BLOCKING_MESSAGE_SELECTOR = 'label:visible, [role="alert"]:visible, p:visible, span:visible';
const SAVE_BLOCKING_MESSAGE_PATTERN = /(verplicht|ongeldig|kan niet|niet toegestaan|ouder\/verzorger)/i;
...
async function clickSaveButton(page) {
  // Changing postcode/house-number fields starts Sportlink's asynchronous
  // address lookup. While that request is in flight the whole action row is
  // removed from the DOM, so an immediate locator count sees no save button.
  // Wait for any explicit save control to return before trying to click it.
  await page.waitForSelector(SAVE_BUTTON_WAIT_SELECTOR, {
    state: 'visible',
    timeout: 8000
  }).catch(() => {});
  ...
  for (const selector of SAVE_BUTTON_SELECTORS) {
    ...
    if (typeof button.isDisabled === 'function' && await button.isDisabled().catch(() => false)) {
      const messages = await page.locator(SAVE_BLOCKING_MESSAGE_SELECTOR).allTextContents().catch(() => []);
      const reason = messages
        .map(message => message.trim())
        .find(message => message.length > 0 && message.length <= 240 && SAVE_BLOCKING_MESSAGE_PATTERN.test(message));
      const error = new Error(reason
        ? `Sportlink save is disabled: ${reason}`
        : 'Sportlink save is disabled by form validation');
      error.code = 'sportlink_validation_blocked';
      throw error;
    }
...
/**
 * Supersede queued values that no longer match the current Rondo Club record.
 * This prevents an intermediate form state from being written after the user
 * has already corrected or reverted that field.
 *
 * @param {Object} db - Database connection
 * @param {Array<Object>} pendingChanges - Pending changes for one member
 * @param {Object} rondoClubData - Current Rondo Club person response
 * @returns {number} Number of obsolete rows superseded
 */
function reconcilePendingChanges(db, pendingChanges, rondoClubData) {
  const knvbId = pendingChanges[0]?.knvb_id;
  if (!knvbId) return 0;

  let superseded = 0;
  const fieldNames = new Set(pendingChanges.map(change => change.field_name));
  for (const fieldName of fieldNames) {
    const currentValue = extractFieldValue(rondoClubData, fieldName);
    superseded += supersedePendingChanges(db, knvbId, fieldName, currentValue);
  }
  return superseded;
}
...
async function navigateWithTimeoutCheck(page, url, credentials, options = {}) {
  ...
  for (let attempt = 1; attempt <= 2; attempt++) {
    ...
    if (isSportlinkAuthUrl(currentUrl)) {
      logger?.verbose('Session expired, re-authenticating...');
      await loginToSportlink(page, { logger, credentials });
    } else {
      logger?.verbose(`Sportlink redirected to ${currentUrl}; retrying requested member page...`);
    }
  }

  throw new Error(`Sportlink did not open the requested member page (ended at ${page.url()})`);
}
...
function dutchDateToISO(dateStr) {
  ...
  const match = dateStr.match(/^(\d{1,2})\s+([a-z]{3,4})\s+(\d{4})$/i);
...
async function syncSinglePage(page, knvbId, pageType, pageChanges, credentials, options = {}) {
  ...
  const phoneFields = ['mobile_1', 'mobile_2', 'telephone_1'];
  ...
  try {
    ...
    const wrapped = new Error(`Could not save on ${pageType} page: ${error.message}`);
...
async function syncMemberMultiPageWithRetry(page, knvbId, pageChanges, credentials, options = {}) {
  ...
  for (let attempt = 0; attempt < maxRetries; attempt++) {
    try {
      ...
      if (/Oud bondslid|Oud verenigingslid/i.test(error.message) || error.code === 'sportlink_validation_blocked') {
        return { success: false, attempts: attempt + 1, error: error.message, errorCode: error.code };
      }
      if (attempt === maxRetries - 1) {
        return { success: false, attempts: attempt + 1, error: error.message, errorCode: error.code };
      }
...
async function runReverseSyncMultiPage(options = {}) {
  ...
  // Re-read every pending member before browser automation. Incremental
  // detection can contain an intermediate form state that has since been
  // corrected; those obsolete rows stay in the audit log as superseded.
  const pendingByMember = new Map();
  for (const change of filteredChanges) {
    if (!pendingByMember.has(change.knvb_id)) pendingByMember.set(change.knvb_id, []);
    pendingByMember.get(change.knvb_id).push(change);
  }

  const deceasedMembers = new Map();
  for (const [memberKnvbId, memberChanges] of pendingByMember) {
    const rondoClubRecord = db.prepare('SELECT rondo_club_id FROM rondo_club_members WHERE knvb_id = ?').get(memberKnvbId);
    if (!rondoClubRecord?.rondo_club_id) continue;

    try {
      const response = await rondoClubRequest(`wp/v2/people/${rondoClubRecord.rondo_club_id}?_fields=fields`, 'GET');
      const superseded = reconcilePendingChanges(db, memberChanges, response.body);
      if (superseded > 0) {
        logger?.log(`Superseded ${superseded} stale pending change(s) for ${memberKnvbId}`);
      }
      const datumOverlijden = response.body?.fields?.['datum_overlijden'];
      if (datumOverlijden && new Date(datumOverlijden) <= new Date()) {
        deceasedMembers.set(memberKnvbId, datumOverlijden);
      }
    } catch (err) {
      logger?.verbose(`Could not refresh current Rondo Club state for ${memberKnvbId}: ${err.message}`);
    }
  }

  const refreshedChanges = getUnsyncedChanges(db);
  const refreshedFilteredChanges = knvbId
    ? refreshedChanges.filter(change => change.knvb_id === knvbId)
    : refreshedChanges;
  const changesByMemberAndPage = groupChangesByMemberAndPage(refreshedFilteredChanges);

  // Filter out deceased members before launching browser
  for (const [memberKnvbId, datumOverlijden] of deceasedMembers) {
    const pageChanges = changesByMemberAndPage.get(memberKnvbId);
    if (!pageChanges) continue;
    const allFieldNames = [
    ...
    markChangesSynced(db, memberKnvbId, allFieldNames);
    const fieldCount = allFieldNames.length;
    logger?.log(`Skipping ${memberKnvbId}: deceased (${datumOverlijden}), marked ${fieldCount} change(s) as synced`);
    changesByMemberAndPage.delete(memberKnvbId);
  }
  ...
  try {
    ...
    for (const [knvbId, pageChanges] of changesByMemberAndPage) {
      ...
      if (result.success) {
        ...
        const rondoRecord = db.prepare('SELECT rondo_club_id FROM rondo_club_members WHERE knvb_id = ?').get(knvbId);
        if (rondoRecord?.rondo_club_id) {
          await notifyProfileChangeStatus(
            rondoRecord.rondo_club_id,
            allFieldNames,
            'synced',
            '',
            { logger }
          ).catch(error => logger?.verbose(`Could not update Rondo wijzigingslogboek: ${error.message}`));
        }

        synced++;
        logger?.log(`Synced ${fieldCount} field(s) for member ${knvbId}`);
      } else if (result.errorCode === 'sportlink_validation_blocked') {
        const allFieldNames = [
          ...pageChanges.general.map(c => c.field_name),
          ...pageChanges.address.map(c => c.field_name),
          ...pageChanges.other.map(c => c.field_name),
        ...
        const deferred = markChangesActionRequired(db, knvbId, allFieldNames, result.error);
        const rondoRecord = db.prepare('SELECT rondo_club_id FROM rondo_club_members WHERE knvb_id = ?').get(knvbId);
        if (deferred.newlyRequired > 0 && rondoRecord?.rondo_club_id) {
          await notifyProfileChangeStatus(
            rondoRecord.rondo_club_id,
            allFieldNames,
            'action_required',
          ...
          await sendActionRequiredAlert({ knvbId, fields: allFieldNames, reason: result.error })
            .catch(error => logger?.verbose(`Could not send action-required alert: ${error.message}`));
        }
        actionRequired++;
        consecutiveEditFailures = 0;
        logger?.log(`Action required for member ${knvbId}: ${result.error}; retry deferred for 24 hours`);
        ...
        if (rondoRecord?.rondo_club_id) {
          await notifyProfileChangeStatus(
            ...
            allFieldNames,
            'failed',
            `Niet schrijfbaar in Sportlink: ${reason}`,
            { logger }
          ).catch(error => logger?.verbose(`Could not update Rondo wijzigingslogboek: ${error.message}`));
        }
        synced++;
        ...
        if (rondoRecord?.rondo_club_id) {
          await notifyProfileChangeStatus(
            ...
            allFieldNames,
            'failed',
            result.error || 'Onbekende Sportlink-fout',
            { logger }
          ).catch(error => logger?.verbose(`Could not update Rondo wijzigingslogboek: ${error.message}`));
        }
      ...
      results.push({
        ...
        error: result.error,
        actionRequired: result.errorCode === 'sportlink_validation_blocked'
      });
  ...
  const success = failed === 0;
  logger?.log(`Multi-page reverse sync complete: ${synced} synced, ${failed} failed, ${actionRequired} action required`);

  return { success, synced, failed, actionRequired, results };
}
