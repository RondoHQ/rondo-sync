...
async function enterEditMode(page, pageType = 'general', expectedSelector = null) {
  ...
  // Wait for the SPA to finish rendering Wijzig buttons. networkidle fires
  // before the React components mount, so counting straight away can see
  // 1-2 fewer buttons than the user does. Wait until the button count is
  // stable across two checks, capped at ~3s. Without this we picked the
  // wrong section (and bailed) for members whose "Wijzig lidsoort" header
  // button rendered before the section-level Wijzig buttons.
  await page.waitForSelector('button:has-text("Wijzig")', { timeout: 5000, state: 'visible' }).catch(() => {});
  let prevCount = -1;
  for (let i = 0; i < 6; i++) {
    const c = await page.locator('button:has-text("Wijzig")').count();
    if (c > 0 && c === prevCount) break;
    prevCount = c;
  ...
  // Some members may have fewer sections, so try multiple indices.
  if (pageType === 'general') {
    attempts.push({ type: 'wijzig-index', index: 2, label: 'Wijzig[2] (communication)' });
    attempts.push({ type: 'wijzig-index', index: 1, label: 'Wijzig[1] (communication fallback)' });
  }

  if (pageType === 'address') {
    attempts.push({ type: 'wijzig-index', index: 3, label: 'Wijzig[3] (address)' });
    attempts.push({ type: 'wijzig-index', index: 2, label: 'Wijzig[2] (address fallback)' });
  }

  if (pageType === 'other') {
    attempts.push({ type: 'wijzig-index', index: 1, label: 'Wijzig[1] (vrije velden)' });
  }

  if (pageType === 'financial') {
    attempts.push({ type: 'wijzig-index', index: 1, label: 'Wijzig[1] (financial)' });
  }
  ...
  // Log how many Wijzig buttons were found for debugging
  const debugButtons = page.locator('button');
  const debugCount = await debugButtons.count();
  const wijzigButtons = [];
  for (let i = 0; i < debugCount; i++) {
    const text = await debugButtons.nth(i).textContent().catch(() => '');
    if (text.includes('Wijzig')) wijzigButtons.push(`[${i}] ${text.trim()}`);
  }

  throw new Error(`Could not find edit button for ${pageType} page (found ${wijzigButtons.length} Wijzig buttons: ${wijzigButtons.join(', ') || 'none'})`);
}
