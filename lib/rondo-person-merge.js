...
/** A retired source must never take ownership of another membership's survivor. */
function hasConflictingKnvbId(knvbId, person) {
  const source = String(knvbId || '').trim();
  const target = String(person?.fields?.knvb_id || '').trim();
