...
async function fetchPeople(request, options) {
  const people = [];
  // Full person serialization on production exceeds PHP limits at 100 records.
  for (let page = 1; ; page++) {
    const response = await request(
      `wp/v2/people?context=edit&status=publish,draft,private,pending,future,trash&orderby=id&order=asc&per_page=25&page=${page}&_fields=id,fields.work_history`,
      'GET', null, options
    );
