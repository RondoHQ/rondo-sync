...
function expectedSavedHistory(person, history, today) {
  // WordPress's FormerMemberWorkHistory hook closes logically current roles on
  // any history save. Verify that existing invariant as well as our own edits.
  if (person.fields?.former_member !== true) return history;
  const membershipEnd = person.fields.lid_tot;
  const endDate = /^\d{4}-\d{2}-\d{2}$/.test(membershipEnd || '') && membershipEnd <= today
    ? membershipEnd : today;
  return history.map(row => {
    if (row.start_date && row.start_date > today) return row;
    const current = row.end_date ? row.end_date > today : row.is_current !== false;
    return current ? { ...row, is_current: false, end_date: endDate } : row;
  });
}

async function fetchPeople(request, options) {
  const people = [];
  // Trashed persons are intentionally inaccessible through the Rondo REST API.
  for (let page = 1; ; page++) {
    const response = await request(
      `wp/v2/people?context=edit&status=publish,draft,private,pending,future&orderby=id&order=asc&per_page=100&page=${page}&_fields=id,fields.work_history`,
      'GET', null, options
    );
...
async function retireMissingTeams(orphans, options = {}) {
  ...
  // Include former members and unpublished persons, but respect deleted-record access.
  // Archived team posts stay intact, including any references held by trashed persons.
  ...
  for (const person of people) {
    ...
    if (!isDeepStrictEqual(historyOf(verified), expectedSavedHistory(current, next, today))) {
...
module.exports = { retireMissingTeams, preserveTeamHistory, hasUnendedRole, expectedSavedHistory };
