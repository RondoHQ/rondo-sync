...
async function buildServer() {
  ...
  const FREE_FIELD_SCOPES = ['acf', 'meta'];
    ...
    try {
      ...
      const mappings = FREE_FIELD_SOURCES.map((source) => {
        ...
        return {
          ...
          target_scope: row?.target_scope || 'acf',
  ...
  app.post('/beheer/free-fields', { preHandler: requireAuth }, async (request, reply) => {
    ...
    const mappings = FREE_FIELD_SOURCES.map((source) => {
      ...
      const scope = String(form[`${key}_scope`] || 'acf').trim();
      const type = String(form[`${key}_type`] || 'string').trim();
      return {
        source_field: source,
        target_field: target || null,
        target_scope: FREE_FIELD_SCOPES.includes(scope) ? scope : 'acf',
