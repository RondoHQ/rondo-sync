...
function initDb(db) {
  ...
  const deliverabilityColumns = db.prepare('PRAGMA table_info(member_deliverability_events)').all();
  const hasIgnoredAt = deliverabilityColumns.some(column => column.name === 'ignored_at');
  if (!hasIgnoredAt) {
    db.exec('ALTER TABLE member_deliverability_events ADD COLUMN ignored_at TEXT');
  }

  const hasIgnoredReason = deliverabilityColumns.some(column => column.name === 'ignored_reason');
  if (!hasIgnoredReason) {
    db.exec('ALTER TABLE member_deliverability_events ADD COLUMN ignored_reason TEXT');
  }

  const hasTodoDeletedAt = deliverabilityColumns.some(column => column.name === 'todo_deleted_at');
  if (!hasTodoDeletedAt) {
    db.exec('ALTER TABLE member_deliverability_events ADD COLUMN todo_deleted_at TEXT');
  }
}
...
/** Stable identity shared by automatic removals and remote deliverability scans. */
function buildLapostaEventKey({ listId, email, memberId, state }) {
  return `${listId}:${memberId || String(email || '').trim().toLowerCase()}:${state}`;
}

/** Journal intentional removals before the API call, including uncertain outcomes. */
function recordAutomaticUnsubscription(db, listIndex, listId, member, reason) {
  const email = String(member.email || member.EmailAddress || '').trim().toLowerCase();
  const eventKey = buildLapostaEventKey({ listId, email, memberId: member.member_id, state: 'unsubscribed' });
  db.transaction(() => {
    upsertMemberDeliverabilityEvents(db, [{
      list_index: listIndex, list_id: listId, member_id: member.member_id,
      email, state: 'unsubscribed', event_key: eventKey,
      payload: { automatic_reason: reason }
    }]);
    const event = db.prepare('SELECT id FROM member_deliverability_events WHERE event_key = ?').get(eventKey);
    markMemberDeliverabilityEventIgnored(db, event.id, reason);
  })();
}

/** Get deliverability events that have not been handled or intentionally ignored. */
...
/**
 * Mark a deliverability event as intentionally ignored (no task should be created).
 *
 * @param {Database} db - Database instance
 * @param {number} eventId - Event row ID
 * @param {string} reason - Ignore reason
 */
function markMemberDeliverabilityEventIgnored(db, eventId, reason) {
  db.prepare(`
    UPDATE member_deliverability_events
    SET
      task_created_at = ?,
...
/**
 * Get bounce events that already created a todo and can be checked for cleanup.
 *
 * @param {Database} db - Database instance
 * @returns {Array} Candidate bounce events
 */
function getBounceDeliverabilityEventsWithTodos(db) {
  return db.prepare(`
    SELECT
      id,
      email,
    ...
    FROM member_deliverability_events
    WHERE state = 'cleaned'
      AND rondo_todo_id IS NOT NULL
      AND todo_deleted_at IS NULL
    ORDER BY id ASC
  `).all();
}

/**
 * Mark a deliverability-linked todo as deleted during cleanup.
 *
 * @param {Database} db - Database instance
 * @param {number} eventId - Event row ID
 */
function markMemberDeliverabilityTodoDeleted(db, eventId) {
  db.prepare(`
    UPDATE member_deliverability_events
    SET todo_deleted_at = ?
    WHERE id = ?
...
function getLatestSportlinkResults(db) {
  return getLatestSportlinkSnapshot(db)?.results_json || null;
}

/** Read the source timestamp together with its payload, never the import time. */
function getLatestSportlinkSnapshot(db) {
  return db.prepare(`
    SELECT created_at, results_json
    FROM sportlink_runs
    ORDER BY id DESC
    LIMIT 1
  `).get() || null;
...
module.exports = {
  ...
  buildLapostaEventKey,
  recordAutomaticUnsubscription,
