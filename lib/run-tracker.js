...
// Retention policy for the `runs` table (see _cleanup).
//
// Three tiers, because one flat age cutoff cannot serve both `reverse` (288
// runs/day) and `functions-full` (one run/week):
//   RECENT_DAYS        - keep everything this fresh, whatever the outcome.
//   FAILURE_DAYS       - keep failure/partial far longer; they are rare, and
//                        "did anything fail recently?" is unanswerable without
//                        them. A flat 3-day sweep made that a 72h question.
//   KEEP_PER_PIPELINE  - floor of newest runs kept per pipeline regardless of
//                        age. This is what makes sparse pipelines watchable:
//                        scripts/heal-sync.sh reads the LATEST run per pipeline,
//                        so the weekly functions-full row must outlive the
//                        3-day window or the watchdog goes permanently blind to
//                        it. It also prevents a retained old failure from
//                        becoming "latest" after the newer success is swept,
//                        which would re-trigger a heal for a fixed episode.
const RETENTION_RECENT_DAYS = 3;
const RETENTION_FAILURE_DAYS = 90;
const RETENTION_KEEP_PER_PIPELINE = 25;
...
class RunTracker {
  ...
  constructor(pipeline) {
    ...
    this.runId = null;
    this.db = null;
    this._lastProgressWrite = new Map(); // stepId -> ms timestamp
  ...
  _prepareStatements() {
    ...
    this._updateStepProgress = this.db.prepare(`
      UPDATE run_steps
      SET progress_current = ?, progress_total = ?, progress_label = ?, progress_updated_at = ?
      WHERE id = ?
    `);
   ...
   *
   * Runs still marked `running` are kept regardless of age — deleting them
   * loses the only record that the dashboard has of a stuck pipeline, so
   * the next launch silently shows "Never run" while the orphaned process
   * still holds its flock. Stale runners get marked `failure` separately
   * (see {@link _markStaleRunning}).
   * @private
   */
  _cleanup() {
    if (!this.db) return;
    ...
    try {
      const now = Date.now();
      const params = {
        recentCutoff: new Date(now - RETENTION_RECENT_DAYS * MS_PER_DAY).toISOString(),
        failureCutoff: new Date(now - RETENTION_FAILURE_DAYS * MS_PER_DAY).toISOString(),
        keepPerPipeline: RETENTION_KEEP_PER_PIPELINE
      };

      // A finished run is purged only if it fails EVERY keep-rule: too old for
      // the recent window, not a still-interesting failure, and not among the
      // newest N for its own pipeline. `running` rows are never touched here —
      // _markStaleRunning() reaps those.
      const doomed = `
        SELECT id FROM runs
        WHERE outcome != 'running'
          AND started_at < @recentCutoff
          AND NOT (outcome IN ('failure', 'partial') AND started_at >= @failureCutoff)
          AND id NOT IN (
            SELECT id FROM (
              ...
              FROM runs
            ) WHERE rn <= @keepPerPipeline
          )
      `;

      // Children first, then parents, atomically — a partial sweep would orphan
      // run_steps/run_errors rows that nothing ever cleans up.
      const sweep = this.db.transaction((p) => {
        this.db.prepare(`DELETE FROM run_errors WHERE run_id IN (${doomed})`).run(p);
        this.db.prepare(`DELETE FROM run_steps WHERE run_id IN (${doomed})`).run(p);
        this.db.prepare(`DELETE FROM runs WHERE id IN (${doomed})`).run(p);
      });
      sweep(params);
    } catch (err) {
  ...
  /**
   * Mark `running` rows older than 12 hours as `failure` so the dashboard
   * stops showing them as in-flight. Runs of this length without a finish
   * timestamp are always crashes (the process exited without endRun), not
   * legitimate long syncs — even player-history finishes within an hour.
   * @private
   */
  _markStaleRunning() {
    if (!this.db) return;
    ...
    try {
      ...
      this.db.prepare(`
        ...
        SET outcome = 'failure',
            finished_at = ?,
            summary_json = COALESCE(summary_json, '{"stale":true,"note":"marked failed by _markStaleRunning"}')
        WHERE outcome = 'running' AND started_at < ?
      `).run(new Date().toISOString(), cutoff);
    } catch (err) {
      console.error(`[run-tracker] Stale-run sweep failed: ${err.message}`);
    }
  }
  ...
  startRun() {
    return this._safe(() => {
      // Mark abandoned runs as failure, then purge old finished rows.
      this._markStaleRunning();
      this._cleanup();
  ...
  /**
   * Update in-flight progress for a step. Writes are throttled to at most
   * once per second per step so a tight loop calling on every iteration is
   * safe. The final position (current === total) always writes through.
   * @param {number|null} stepId - Step ID from startStep
   * @param {Object} options
   * @param {number} options.current - Items processed so far
   * @param {number} options.total - Total items
   * @param {string} [options.label] - Optional label (e.g., current item identifier)
   */
  updateStep(stepId, { current, total, label = null } = {}) {
    if (!stepId || typeof current !== 'number' || typeof total !== 'number') return;

    this._safe(() => {
      const now = Date.now();
      const last = this._lastProgressWrite.get(stepId) || 0;
      const isFinal = current >= total;
      if (!isFinal && now - last < 1000) return;

      this._lastProgressWrite.set(stepId, now);
      this._updateStepProgress.run(current, total, label, nowISO(), stepId);
    }, 'updateStep');
  }
  ...
  endStep(stepId, options = {}) {
    ...
    this._safe(() => {
      ...
      this._lastProgressWrite.delete(stepId);
    }, 'endStep');
  }
