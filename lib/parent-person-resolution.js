...
/** Keep an existing name when this source only has a generated parent label. */
function resolveParentImportName(incoming, existing, preserveIdentity = false) {
  const fallback = String(incoming?.first_name || '').startsWith('Ouder van ');
  const hasExisting = Boolean(String(existing?.first_name || '').trim() || String(existing?.last_name || '').trim());
  if (preserveIdentity || (fallback && hasExisting)) {
    return { first_name: existing?.first_name || '', last_name: existing?.last_name || '' };
  }
  return { first_name: incoming?.first_name || existing?.first_name || '', last_name: incoming?.last_name || existing?.last_name || '' };
}
