 ...
 *
 * Empty/non-applicable people are added later from the local identity tables.
 ...
 * People without an obligation deliberately map to an empty value.
 */
function buildRecipientObligationMaps(personValues, memberRows, parentRows) {
  ...
  for (const row of memberRows) {
    ...
    byKnvbId.set(String(row.knvb_id), personValues.has(personId) ? personValues.get(personId) : '');
  }
  ...
  for (const row of parentRows) {
    ...
    if (!email) continue;
    const personId = row.rondo_club_id ? String(row.rondo_club_id) : '';
    byParentEmail.set(email, personValues.has(personId) ? personValues.get(personId) : '');
  }
...
function resolveLapostaObligationValue(maps, { knvbId, email, emailType }) {
  ...
  return maps.byKnvbId.has(normalizedKnvbId) ? maps.byKnvbId.get(normalizedKnvbId) : '';
}
