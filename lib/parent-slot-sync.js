/**
 * Reverse-sync parent/guardian relationships from Rondo Club to Sportlink.
 *
 * The implementation lives in one module so detection, queueing and the
 * browser writer share the same normalization rules and safety invariants.
 */

const { SportlinkSession } = require('./sportlink-session');
const { openDb } = require('./rondo-club-db');
const { rondoClubRequest, notifyProfileChangeStatus } = require('./rondo-club-client');
const { enterEditMode, clickSaveButton, navigateWithTimeoutCheck } = require('./reverse-sync-sportlink');
const { stableStringify, computeHash } = require('./utils');

const PARENT_INFO_PATH = '/navajo/entity/common/clubweb/member/MemberParentalInfo';
const MAX_ATTEMPTS = 5;
const EMAIL_SYNC_FIELDS = ['email_1', 'email_2'];

function normalizeEmail(value) {
  return String(value || '').trim().toLowerCase();
}

function isValidEmail(value) {
  return /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(normalizeEmail(value));
}

function normalizeText(value) {
  return String(value || '').trim().replace(/\s+/g, ' ');
}

function normalizeName(value) {
  return normalizeText(value).toLowerCase().replace(/\s*-\s*/g, '-');
}

function normalizePhone(value) {
  let digits = String(value || '').replace(/\D/g, '');
  if (digits.startsWith('0031')) digits = `0${digits.slice(4)}`;
  else if (digits.startsWith('31')) digits = `0${digits.slice(2)}`;
  return digits;
}

function ensureParentSyncSchema(db) {
  db.exec(`
    CREATE TABLE IF NOT EXISTS parent_slot_sync_state (
      id INTEGER PRIMARY KEY CHECK (id = 1),
      last_detection_at TEXT NOT NULL,
      updated_at TEXT NOT NULL
    );

    CREATE TABLE IF NOT EXISTS parent_slot_sync_jobs (
      id INTEGER PRIMARY KEY,
      child_knvb_id TEXT NOT NULL,
      child_rondo_id INTEGER NOT NULL,
      parent_rondo_id INTEGER NOT NULL,
      desired_json TEXT NOT NULL,
      desired_hash TEXT NOT NULL,
      state TEXT NOT NULL DEFAULT 'pending',
      slot INTEGER,
      attempts INTEGER NOT NULL DEFAULT 0,
      last_error TEXT,
      next_attempt_at TEXT,
      created_at TEXT NOT NULL,
      updated_at TEXT NOT NULL,
      synced_at TEXT,
      UNIQUE (child_rondo_id, parent_rondo_id),
      CHECK (state IN ('pending', 'retry', 'blocked', 'synced', 'cancelled')),
      CHECK (slot IS NULL OR slot IN (1, 2))
    );

    CREATE INDEX IF NOT EXISTS idx_parent_slot_sync_jobs_ready
      ON parent_slot_sync_jobs (state, next_attempt_at, updated_at);
  `);
}

function getLastParentDetectionTime(db) {
  ensureParentSyncSchema(db);
  return db.prepare('SELECT last_detection_at FROM parent_slot_sync_state WHERE id = 1').get()?.last_detection_at || null;
}

function updateLastParentDetectionTime(db, timestamp) {
  ensureParentSyncSchema(db);
  const now = new Date().toISOString();
  db.prepare(`
    INSERT INTO parent_slot_sync_state (id, last_detection_at, updated_at)
    VALUES (1, ?, ?)
    ON CONFLICT(id) DO UPDATE SET
      last_detection_at = excluded.last_detection_at,
      updated_at = excluded.updated_at
  `).run(timestamp, now);
}

function relationshipSlug(relationship) {
  return String(relationship?.relationship_slug || '').trim().toLowerCase();
}

function relatedPersonId(relationship) {
  return Number(relationship?.related_person_id || 0);
}

function formatPersonName(fields = {}) {
  return normalizeText([fields.first_name, fields.infix, fields.last_name].filter(Boolean).join(' '));
}

function buildDesiredParent(parent, child) {
  const fields = parent?.fields || {};
  const childFields = child?.fields || {};
  const email = normalizeEmail(fields.email_1 || fields.email_2);
  const desired = {
    childKnvbId: normalizeText(childFields.knvb_id),
    childRondoId: Number(child?.id || 0),
    parentRondoId: Number(parent?.id || 0),
    name: formatPersonName(fields),
    email,
    phone: normalizeText(fields.mobile_1 || fields.telephone_1 || fields.mobile_2 || fields.telephone_2)
  };

  if (!desired.childKnvbId || !desired.childRondoId || !desired.parentRondoId) {
    throw new Error('Kind of ouder mist een stabiele Rondo/Sportlink-identificatie.');
  }
  if (!desired.name) {
    throw new Error('De ouder/verzorger heeft geen naam.');
  }
  if (!isValidEmail(desired.email)) {
    throw new Error('De ouder/verzorger heeft geen geldig e-mailadres.');
  }

  return desired;
}

function findTrackedParentSource(db, parentRondoId, childKnvbId) {
  const rows = db.prepare(`
    SELECT email, data_json
    FROM rondo_club_parents
    WHERE rondo_club_id = ?
    ORDER BY last_seen_at DESC, id DESC
  `).all(parentRondoId);

  for (const row of rows) {
    let childIds = [];
    try {
      childIds = JSON.parse(row.data_json || '{}').childKnvbIds || [];
    } catch {
      continue;
    }
    if (childIds.map(String).includes(String(childKnvbId))) {
      const email = normalizeEmail(row.email);
      if (email) return { email };
    }
  }
  return null;
}

function desiredHash(desired) {
  return computeHash(stableStringify(desired));
}

function upsertParentJob(db, desired) {
  ensureParentSyncSchema(db);
  const now = new Date().toISOString();
  const hash = desiredHash(desired);
  const existing = db.prepare(`
    SELECT id, desired_hash, state FROM parent_slot_sync_jobs
    WHERE child_rondo_id = ? AND parent_rondo_id = ?
  `).get(desired.childRondoId, desired.parentRondoId);

  if (existing?.desired_hash === hash && existing.state !== 'cancelled') {
    return { id: existing.id, changed: false, state: existing.state };
  }

  db.prepare(`
    INSERT INTO parent_slot_sync_jobs (
      child_knvb_id, child_rondo_id, parent_rondo_id, desired_json,
      desired_hash, state, attempts, last_error, next_attempt_at,
      created_at, updated_at, synced_at, slot
    ) VALUES (?, ?, ?, ?, ?, 'pending', 0, NULL, NULL, ?, ?, NULL, NULL)
    ON CONFLICT(child_rondo_id, parent_rondo_id) DO UPDATE SET
      child_knvb_id = excluded.child_knvb_id,
      desired_json = excluded.desired_json,
      desired_hash = excluded.desired_hash,
      state = 'pending',
      attempts = 0,
      last_error = NULL,
      next_attempt_at = NULL,
      updated_at = excluded.updated_at,
      synced_at = NULL,
      slot = NULL
  `).run(
    desired.childKnvbId,
    desired.childRondoId,
    desired.parentRondoId,
    stableStringify(desired),
    hash,
    now,
    now
  );

  const job = db.prepare(`
    SELECT id, state FROM parent_slot_sync_jobs
    WHERE child_rondo_id = ? AND parent_rondo_id = ?
  `).get(desired.childRondoId, desired.parentRondoId);
  return { id: job.id, changed: true, state: job.state };
}

function cancelMissingParentJobs(db, childRondoId, desiredParentIds) {
  ensureParentSyncSchema(db);
  const ids = Array.from(desiredParentIds);
  const now = new Date().toISOString();
  if (ids.length === 0) {
    return db.prepare(`
      UPDATE parent_slot_sync_jobs SET state = 'cancelled', updated_at = ?
      WHERE child_rondo_id = ? AND state IN ('pending', 'retry', 'blocked')
    `).run(now, childRondoId).changes;
  }
  const placeholders = ids.map(() => '?').join(', ');
  return db.prepare(`
    UPDATE parent_slot_sync_jobs SET state = 'cancelled', updated_at = ?
    WHERE child_rondo_id = ?
      AND parent_rondo_id NOT IN (${placeholders})
      AND state IN ('pending', 'retry', 'blocked')
  `).run(now, childRondoId, ...ids).changes;
}

async function fetchModifiedPeople(since, options = {}) {
  const people = [];
  let page = 1;
  while (true) {
    const endpoint = `wp/v2/people?per_page=100&page=${page}&modified_after=${encodeURIComponent(since)}&_fields=id,modified_gmt,fields,parent_sync_statuses`;
    const response = await rondoClubRequest(endpoint, 'GET', null, options);
    if (!Array.isArray(response.body)) throw new Error('Unexpected Rondo Club people response');
    people.push(...response.body);
    if (response.body.length < 100) break;
    page++;
  }
  return people;
}

async function fetchPerson(personId, options = {}) {
  const response = await rondoClubRequest(`wp/v2/people/${personId}?_fields=id,status,modified_gmt,fields,parent_sync_statuses`, 'GET', null, options);
  return response.body;
}

async function fetchProfileChangesSince(since, options = {}) {
  const entries = [];
  let page = 1;
  while (true) {
    const response = await rondoClubRequest(
      `rondo/v1/profile-change-log?page=${page}&per_page=100`,
      'GET',
      null,
      options
    );
    const body = response.body || {};
    const items = Array.isArray(body.items) ? body.items : [];
    for (const item of items) {
      if (new Date(item.created_at).getTime() >= new Date(since).getTime()) entries.push(item);
    }
    const reachedOlderEntries = items.some(item => new Date(item.created_at).getTime() < new Date(since).getTime());
    if (items.length === 0 || reachedOlderEntries || page >= Number(body.total_pages || 1)) break;
    page++;
  }
  return entries.sort((left, right) => new Date(left.created_at) - new Date(right.created_at));
}

function extractEmailReplacementCandidates(entries) {
  const candidates = [];
  for (const entry of entries) {
    if (entry.sync_status !== 'pending') continue;
    const changes = Array.isArray(entry.changes) ? entry.changes : [];
    for (const change of changes) {
      if (change.parent_sync) continue; // New audits have independently tracked child targets.
      const field = String(change.field || '');
      if (!EMAIL_SYNC_FIELDS.includes(field) || change.sync !== true) continue;
      if (entry.type === 'email_promoted' && field !== 'email_1') continue;

      const oldEmail = normalizeEmail(change.old);
      const newEmail = normalizeEmail(change.new);
      if (!isValidEmail(oldEmail)) continue;
      if (newEmail && !isValidEmail(newEmail)) continue;

      candidates.push({
        auditId: Number(entry.id || 0),
        createdAt: String(entry.created_at || ''),
        personId: Number(change.person_id || 0),
        oldEmail,
        newEmail,
        field,
        type: String(entry.type || '')
      });
    }
  }
  return candidates.filter(candidate => candidate.auditId && candidate.personId);
}

function resolveEmailReplacement(candidate, parent) {
  const fields = parent?.fields || {};
  const currentEmails = [fields.email_1, fields.email_2]
    .map(normalizeEmail)
    .filter(isValidEmail);
  const newEmail = candidate.newEmail || currentEmails[0] || '';
  if (!isValidEmail(newEmail) || !currentEmails.includes(newEmail)) return null;
  if (candidate.oldEmail === newEmail) return null;
  return { ...candidate, newEmail };
}

async function reportParentStatus(job, state, details = {}, options = {}) {
  const payload = {
    parent_id: Number(job.parent_rondo_id || job.parentRondoId),
    state,
    slot: details.slot || null,
    message: details.message || ''
  };
  try {
    await rondoClubRequest(
      `rondo/v1/people/${job.child_rondo_id || job.childRondoId}/parent-sync-status`,
      'POST',
      payload,
      options
    );
  } catch (error) {
    options.logger?.verbose(`Kon ouder-syncstatus niet terugmelden: ${error.message}`);
  }
}

async function reconcileChild(db, child, options = {}) {
  const fields = child?.fields || {};
  if (!fields.knvb_id || fields.former_member === true) return { queued: 0, blocked: 0 };

  const statusesByParentId = new Map(
    (child.parent_sync_statuses || [])
      .filter(status => ['pending', 'error'].includes(String(status.state || '')))
      .map(status => [Number(status.parent_id), status])
      .filter(([parentId]) => parentId)
  );
  const allParentIds = new Set();
  for (const relationship of fields.relationships || []) {
    if (relationshipSlug(relationship) !== 'parent') continue;
    const parentId = relatedPersonId(relationship);
    if (!parentId) continue;
    allParentIds.add(parentId);
  }
  cancelMissingParentJobs(db, Number(child.id), allParentIds);
  const parentIds = new Set([...allParentIds].filter(parentId => statusesByParentId.has(parentId)));

  let queued = 0;
  let blocked = 0;
  for (const parentId of parentIds) {
    const contactJob = db.prepare('SELECT desired_json FROM parent_slot_sync_jobs WHERE child_rondo_id = ? AND parent_rondo_id = ? AND state IN (\'pending\', \'retry\', \'blocked\')').get(child.id, parentId);
    if (contactJob && JSON.parse(contactJob.desired_json).mode === 'replace_contact') continue;
    let jobRef = { childRondoId: child.id, parentRondoId: parentId };
    try {
      const personFetcher = options.fetchPerson || fetchPerson;
      const parent = await personFetcher(parentId, options);
      const desired = buildDesiredParent(parent, child);
      const result = upsertParentJob(db, desired);
      jobRef = desired;
      if (result.changed) {
        queued++;
        const reporter = options.reportParentStatus || reportParentStatus;
        await reporter(jobRef, 'pending', {}, options);
      }
    } catch (error) {
      blocked++;
      const desired = {
        childKnvbId: normalizeText(fields.knvb_id),
        childRondoId: Number(child.id),
        parentRondoId: parentId,
        name: '', email: '', phone: ''
      };
      upsertParentJob(db, desired);
      db.prepare(`
        UPDATE parent_slot_sync_jobs
        SET state = 'blocked', last_error = ?, updated_at = ?
        WHERE child_rondo_id = ? AND parent_rondo_id = ?
      `).run(error.message, new Date().toISOString(), child.id, parentId);
      const reporter = options.reportParentStatus || reportParentStatus;
      await reporter(jobRef, 'error', { message: error.message }, options);
    }
  }
  return { queued, blocked };
}

async function reconcileParentEmailChanges(db, entries, peopleById = new Map(), options = {}) {
  const candidates = extractEmailReplacementCandidates(entries);
  let queued = 0;
  let blocked = 0;
  const personFetcher = options.fetchPerson || fetchPerson;
  const reporter = options.reportParentStatus || reportParentStatus;

  for (const candidate of candidates) {
    const parent = peopleById.get(candidate.personId) || await personFetcher(candidate.personId, options);
    const replacement = resolveEmailReplacement(candidate, parent);
    if (!replacement) continue;

    for (const relationship of parent?.fields?.relationships || []) {
      if (relationshipSlug(relationship) !== 'child') continue;
      const childId = relatedPersonId(relationship);
      if (!childId) continue;
      const child = peopleById.get(childId) || await personFetcher(childId, options);
      const childFields = child?.fields || {};
      if (!childFields.knvb_id || childFields.former_member === true) continue;
      const stillLinked = (childFields.relationships || []).some(childRelationship => (
        relationshipSlug(childRelationship) === 'parent'
          && relatedPersonId(childRelationship) === candidate.personId
      ));
      if (!stillLinked) continue;

      const desired = {
        mode: 'replace_email',
        childKnvbId: normalizeText(childFields.knvb_id),
        childRondoId: Number(child.id),
        parentRondoId: candidate.personId,
        name: formatPersonName(parent.fields),
        email: replacement.newEmail,
        sourceEmail: replacement.oldEmail,
        auditId: replacement.auditId
      };
      try {
        const result = upsertParentJob(db, desired);
        if (result.changed) {
          queued++;
          await reporter(desired, 'pending', {}, options);
        }
      } catch (error) {
        blocked++;
        options.logger?.error(`Ouder-e-mailtaak voor ${childFields.knvb_id} kon niet worden klaargezet: ${error.message}`);
      }
    }
  }
  return { queued, blocked };
}

/** Queue only the contact values explicitly changed in a parent self-service audit. */
async function reconcileParentContactChanges(db, entries, peopleById = new Map(), options = {}) {
  const fetcher = options.fetchPerson || fetchPerson;
  const getPerson = async id => {
    if (!peopleById.has(id)) peopleById.set(id, await fetcher(id, options));
    return peopleById.get(id);
  };
  const plans = new Map();
  for (const entry of entries) {
    if (entry.sync_status !== 'pending') continue;
    for (const change of entry.changes || []) {
      const route = change.parent_sync;
      if (!route || !['email', 'phone'].includes(route.kind)) continue;
      const parent = await getPerson(Number(change.person_id));
      for (const childId of route.child_ids || []) {
        const child = await getPerson(Number(childId));
        const linked = (parent.fields?.relationships || []).some(r => relationshipSlug(r) === 'child' && relatedPersonId(r) === Number(childId))
          && (child.fields?.relationships || []).some(r => relationshipSlug(r) === 'parent' && relatedPersonId(r) === Number(parent.id));
        const syncField = `parent_${childId}_${change.field}`;
        if (!linked || !child.fields?.knvb_id || child.fields.former_member === true) {
          await (options.notifyProfileChangeStatus || notifyProfileChangeStatus)(parent.id, [syncField], 'action_required', 'De gekoppelde ouder/kindrelatie is niet meer beschikbaar voor Sportlink.', options);
          continue;
        }
        const key = `${parent.id}:${childId}`;
        if (!plans.has(key)) {
          const existing = db.prepare('SELECT desired_json, state FROM parent_slot_sync_jobs WHERE child_rondo_id = ? AND parent_rondo_id = ?').get(childId, parent.id);
          const previous = existing ? JSON.parse(existing.desired_json) : {};
          plans.set(key, {
            mode: 'replace_contact', childKnvbId: child.fields.knvb_id,
            childRondoId: Number(childId), parentRondoId: Number(parent.id),
            name: formatPersonName(parent.fields),
            identityEmails: [parent.fields.email_1, parent.fields.email_2].map(normalizeEmail).filter(isValidEmail),
            changes: previous.mode === 'replace_contact' && ['pending', 'retry', 'blocked'].includes(existing.state) ? previous.changes : []
          });
        }
        const plan = plans.get(key);
        if (!plan.changes.some(c => c.auditId === entry.id && c.syncField === syncField)) {
          const target = route.kind === 'email' && !route.new
            ? normalizeEmail(parent.fields.email_1 || parent.fields.email_2) : route.new;
          plan.changes.push({ auditId: entry.id, syncField, kind: route.kind, old: route.old, new: target });
        }
      }
    }
  }
  let queued = 0;
  for (const plan of plans.values()) {
    if (upsertParentJob(db, plan).changed) {
      queued++;
      await (options.reportParentStatus || reportParentStatus)(plan, 'pending', {}, options);
    }
  }
  return { queued, blocked: 0 };
}

/** Preserve unedited slot fields and reject ambiguous identities or conflicting old values. */
function planParentContactReplacement(slots, desired) {
  const emails = new Set([
    ...desired.identityEmails,
    ...desired.changes.filter(c => c.kind === 'email').flatMap(c => [normalizeEmail(c.old), normalizeEmail(c.new)])
  ].filter(isValidEmail));
  const matches = slots.filter(slot => normalizeName(slot.name) === normalizeName(desired.name) && emails.has(normalizeEmail(slot.email)));
  if (matches.length !== 1) throw Object.assign(new Error('De bestaande ouderplaats is niet eenduidig gevonden; controleer naam en e-mailadres.'), { code: 'parent_contact_conflict' });
  const before = matches[0];
  const target = { ...before };
  for (const kind of ['email', 'phone']) {
    const changes = desired.changes.filter(c => c.kind === kind);
    if (!changes.length) continue;
    const normalize = kind === 'email' ? normalizeEmail : normalizePhone;
    const finalValue = changes.at(-1).new;
    // A retry after a successful save needs no additional write.
    if (normalize(target[kind]) === normalize(finalValue)) continue;
    for (const change of changes) {
      if (normalize(target[kind]) === normalize(change.old)) target[kind] = change.new;
    }
    if (kind === 'email' && target.email === before.email && desired.identityEmails.includes(normalizeEmail(before.email))) continue;
    if (normalize(target[kind]) !== normalize(finalValue)) {
      throw Object.assign(new Error('De huidige oudercontactgegevens in Sportlink wijken af van de vastgelegde oude waarde.'), { code: 'parent_contact_conflict' });
    }
  }
  return { before, target, slot: before.slot, alreadySynced: parentValuesMatch(before, target) };
}

async function validateParentContactJob(job, desired, options = {}) {
  const fetcher = options.fetchPerson || fetchPerson;
  const [parent, child] = await Promise.all([fetcher(job.parent_rondo_id, options), fetcher(job.child_rondo_id, options)]);
  const linked = (parent.fields?.relationships || []).some(r => relationshipSlug(r) === 'child' && relatedPersonId(r) === Number(child.id))
    && (child.fields?.relationships || []).some(r => relationshipSlug(r) === 'parent' && relatedPersonId(r) === Number(parent.id));
  if (!linked || parent.status !== 'publish' || child.status !== 'publish' || child.fields?.knvb_id !== job.child_knvb_id || child.fields.former_member === true) {
    throw Object.assign(new Error('De ouder/kindrelatie is gewijzigd; controleer de synchronisatietaak.'), { code: 'parent_contact_conflict' });
  }
  for (const kind of ['email', 'phone']) {
    const changes = desired.changes.filter(c => c.kind === kind);
    if (!changes.length) continue;
    const normalize = kind === 'email' ? normalizeEmail : normalizePhone;
    const values = kind === 'email' ? [parent.fields.email_1, parent.fields.email_2]
      : [parent.fields.mobile_1 || parent.fields.telephone_1 || parent.fields.mobile_2 || parent.fields.telephone_2 || ''];
    if (!values.map(normalize).includes(normalize(changes.at(-1).new))) {
      throw Object.assign(new Error('De oudercontactgegevens zijn intussen gewijzigd; controleer de synchronisatietaak.'), { code: 'parent_contact_conflict' });
    }
  }
}

async function detectParentChanges(options = {}) {
  const { logger } = options;
  const db = openDb();
  ensureParentSyncSchema(db);
  const startedAt = new Date().toISOString();
  try {
    const lastDetection = getLastParentDetectionTime(db);
    if (!lastDetection) {
      updateLastParentDetectionTime(db, startedAt);
      logger?.log('Ouder-syncdetector geïnitialiseerd; toekomstige wijzigingen worden gevolgd');
      return { candidates: 0, queued: 0, blocked: 0, initialized: true };
    }

    const overlapSince = new Date(new Date(lastDetection).getTime() - 5000).toISOString();
    const [modified, profileChanges] = await Promise.all([
      fetchModifiedPeople(overlapSince, options),
      fetchProfileChangesSince(overlapSince, options)
    ]);
    const peopleById = new Map(modified.map(person => [Number(person.id), person]));
    const candidateChildIds = new Set();

    for (const person of modified) {
      const fields = person.fields || {};
      if (fields.knvb_id) candidateChildIds.add(Number(person.id));
    }

    let queued = 0;
    let blocked = 0;
    for (const childId of candidateChildIds) {
      const child = peopleById.get(childId) || await fetchPerson(childId, options);
      const result = await reconcileChild(db, child, options);
      queued += result.queued;
      blocked += result.blocked;
    }
    const emailResult = await reconcileParentEmailChanges(db, profileChanges, peopleById, options);
    queued += emailResult.queued;
    blocked += emailResult.blocked;
    const contactResult = await reconcileParentContactChanges(db, profileChanges, peopleById, options);
    queued += contactResult.queued;
    blocked += contactResult.blocked;
    updateLastParentDetectionTime(db, startedAt);
    logger?.log(`Ouder-sync: ${candidateChildIds.size} kind(eren) bekeken, ${queued} taak/taken klaar, ${blocked} geblokkeerd`);
    return { candidates: candidateChildIds.size, queued, blocked, initialized: false };
  } finally {
    db.close();
  }
}

function findParentInfoObject(value) {
  if (!value || typeof value !== 'object') return null;
  if (Object.hasOwn(value, 'NameParent1') || Object.hasOwn(value, 'EmailAddressParent1')) return value;
  for (const child of Object.values(value)) {
    const match = findParentInfoObject(child);
    if (match) return match;
  }
  return null;
}

function extractParentSlots(payload) {
  const info = findParentInfoObject(payload) || {};
  return [1, 2].map(slot => ({
    slot,
    name: normalizeText(info[`NameParent${slot}`]),
    email: normalizeEmail(info[`EmailAddressParent${slot}`]),
    phone: normalizeText(info[`TelephoneParent${slot}`])
  }));
}

function selectParentSlot(slots, desired) {
  const email = normalizeEmail(desired.email);
  const emailMatches = slots.filter(slot => slot.email && slot.email === email);
  if (emailMatches.length === 1) return { slot: emailMatches[0].slot, existing: true };

  const sourceEmail = normalizeEmail(desired.sourceEmail);
  if (sourceEmail) {
    const sourceMatches = slots.filter(slot => slot.email && slot.email === sourceEmail);
    if (sourceMatches.length === 1) return { slot: sourceMatches[0].slot, existing: true };
  }

  const compatible = slots.filter(slot => {
    const existing = {
      name: normalizeName(slot.name),
      email: normalizeEmail(slot.email),
      phone: normalizePhone(slot.phone)
    };
    if (!existing.name && !existing.email && !existing.phone) return false;
    if (existing.name && existing.name !== normalizeName(desired.name)) return false;
    if (existing.email && existing.email !== email) return false;
    if (existing.phone && existing.phone !== normalizePhone(desired.phone)) return false;
    return true;
  });
  if (compatible.length === 1) return { slot: compatible[0].slot, existing: true };

  const empty = slots.find(slot => !slot.name && !slot.email && !slot.phone);
  if (empty) return { slot: empty.slot, existing: false };
  return null;
}

function slotNameMatches(slot, desired) {
  return !normalizeName(slot.name) || normalizeName(slot.name) === normalizeName(desired.name);
}

function selectEmailReplacementSlot(slots, desired) {
  const sourceEmail = normalizeEmail(desired.sourceEmail);
  const sourceMatches = slots.filter(slot => slot.email === sourceEmail && slotNameMatches(slot, desired));
  if (sourceMatches.length === 1) {
    return { slot: sourceMatches[0].slot, existing: true, alreadyTarget: false };
  }
  if (sourceMatches.length > 1) return null;

  const targetEmail = normalizeEmail(desired.email);
  const targetMatches = slots.filter(slot => slot.email === targetEmail && slotNameMatches(slot, desired));
  if (targetMatches.length === 1) {
    return { slot: targetMatches[0].slot, existing: true, alreadyTarget: true };
  }
  return null;
}

function parentValuesMatch(slot, desired) {
  return normalizeName(slot.name) === normalizeName(desired.name)
    && normalizeEmail(slot.email) === normalizeEmail(desired.email)
    && normalizePhone(slot.phone) === normalizePhone(desired.phone);
}

function emailReplacementMatches(slot, before, desired) {
  return normalizeEmail(slot.email) === normalizeEmail(desired.email)
    && normalizeName(slot.name) === normalizeName(before.name)
    && normalizePhone(slot.phone) === normalizePhone(before.phone);
}

async function readParentInfo(page, knvbId) {
  const url = `https://club.sportlink.com/member/member-details/${knvbId}/general`;
  const credentials = {
    username: process.env.SPORTLINK_USERNAME,
    password: process.env.SPORTLINK_PASSWORD,
    otpSecret: process.env.SPORTLINK_OTP_SECRET
  };
  const [response] = await Promise.all([
    page.waitForResponse(
      candidate => candidate.url().includes('/member/MemberParentalInfo?') && candidate.request().method() === 'GET',
      { timeout: 15000 }
    ),
    navigateWithTimeoutCheck(page, url, credentials)
  ]);
  if (!response.ok()) throw new Error(`Sportlink oudergegevens lezen mislukte (HTTP ${response.status()})`);
  return response.json();
}

async function writeParentJob(page, job, options = {}) {
  const desired = JSON.parse(job.desired_json);
  const payload = await readParentInfo(page, job.child_knvb_id);
  const slots = extractParentSlots(payload);
  if (desired.mode === 'replace_contact') {
    const plan = planParentContactReplacement(slots, desired);
    if (plan.alreadySynced) return { slot: plan.slot, alreadySynced: true };
    await enterEditMode(page, 'parent', 'input[name="NameParent1"]');
    for (const [field, input] of [['email', 'EmailAddressParent'], ['phone', 'TelephoneParent']]) {
      if (plan.target[field] !== plan.before[field]) await page.locator(`input[name="${input}${plan.slot}"]`).fill(plan.target[field] || '');
    }
    const [saved] = await Promise.all([
      page.waitForResponse(response => response.url().includes(PARENT_INFO_PATH) && response.request().method() === 'PUT', { timeout: 15000 }),
      clickSaveButton(page)
    ]);
    if (!saved.ok()) throw new Error(`Sportlink oudergegevens opslaan mislukte (HTTP ${saved.status()})`);
    const verified = extractParentSlots(await readParentInfo(page, job.child_knvb_id));
    if (!parentValuesMatch(verified.find(slot => slot.slot === plan.slot) || {}, plan.target)) throw new Error('Sportlink bevestigde de opgeslagen oudergegevens niet.');
    return { slot: plan.slot, alreadySynced: false };
  }
  const emailOnly = desired.mode === 'replace_email';
  const selection = emailOnly
    ? selectEmailReplacementSlot(slots, desired)
    : selectParentSlot(slots, desired);
  if (!selection) {
    const error = new Error(emailOnly
      ? 'De bestaande ouderplaats met het gewijzigde e-mailadres is niet eenduidig gevonden.'
      : 'Beide ouder/verzorger-velden in Sportlink zijn al bezet.');
    error.code = emailOnly ? 'no_matching_parent_email' : 'no_free_parent_slot';
    throw error;
  }

  const current = slots.find(slot => slot.slot === selection.slot);
  if (emailOnly && selection.alreadyTarget) {
    return { slot: selection.slot, alreadySynced: true };
  }
  if (!emailOnly && selection.existing && parentValuesMatch(current, desired)) {
    return { slot: selection.slot, alreadySynced: true };
  }

  await enterEditMode(page, 'parent', 'input[name="NameParent1"]');
  if (!emailOnly) {
    await page.locator(`input[name="NameParent${selection.slot}"]`).fill(desired.name);
  }
  await page.locator(`input[name="EmailAddressParent${selection.slot}"]`).fill(desired.email);
  if (!emailOnly) {
    await page.locator(`input[name="TelephoneParent${selection.slot}"]`).fill(desired.phone || '');
  }

  const [saveResponse] = await Promise.all([
    page.waitForResponse(
      response => response.url().includes(PARENT_INFO_PATH) && response.request().method() === 'PUT',
      { timeout: 15000 }
    ),
    clickSaveButton(page)
  ]);
  if (!saveResponse.ok()) throw new Error(`Sportlink oudergegevens opslaan mislukte (HTTP ${saveResponse.status()})`);

  const verified = extractParentSlots(await readParentInfo(page, job.child_knvb_id));
  const verifiedSlot = verified.find(slot => slot.slot === selection.slot);
  const verifiedMatch = emailOnly
    ? emailReplacementMatches(verifiedSlot || {}, current, desired)
    : parentValuesMatch(verifiedSlot || {}, desired);
  if (!verifiedSlot || !verifiedMatch) {
    throw new Error('Sportlink bevestigde de opgeslagen oudergegevens niet.');
  }
  return { slot: selection.slot, alreadySynced: false };
}

function getReadyParentJobs(db, limit = 20) {
  ensureParentSyncSchema(db);
  return db.prepare(`
    SELECT * FROM parent_slot_sync_jobs
    WHERE state IN ('pending', 'retry')
      AND (next_attempt_at IS NULL OR next_attempt_at <= ?)
    ORDER BY created_at ASC
    LIMIT ?
  `).all(new Date().toISOString(), limit);
}

function markParentJobSynced(db, jobId, slot) {
  const now = new Date().toISOString();
  db.prepare(`
    UPDATE parent_slot_sync_jobs
    SET state = 'synced', slot = ?, last_error = NULL, next_attempt_at = NULL,
        synced_at = ?, updated_at = ?
    WHERE id = ?
  `).run(slot, now, now, jobId);
}

function hasUnresolvedParentJobs(db, parentRondoId) {
  return Boolean(db.prepare(`
    SELECT 1
    FROM parent_slot_sync_jobs
    WHERE parent_rondo_id = ?
      AND state IN ('pending', 'retry', 'blocked')
    LIMIT 1
  `).get(parentRondoId));
}

function markParentJobFailed(db, job, error) {
  const attempts = Number(job.attempts || 0) + 1;
  const blocked = ['no_free_parent_slot', 'no_matching_parent_email', 'parent_contact_conflict'].includes(error.code)
    || attempts >= MAX_ATTEMPTS;
  const delayMinutes = Math.min(60, 2 ** attempts);
  const nextAttempt = blocked ? null : new Date(Date.now() + delayMinutes * 60000).toISOString();
  db.prepare(`
    UPDATE parent_slot_sync_jobs
    SET state = ?, attempts = ?, last_error = ?, next_attempt_at = ?, updated_at = ?
    WHERE id = ?
  `).run(blocked ? 'blocked' : 'retry', attempts, error.message, nextAttempt, new Date().toISOString(), job.id);
  return blocked;
}

async function runParentSlotSync(options = {}) {
  const { logger } = options;
  const db = openDb();
  ensureParentSyncSchema(db);
  const jobs = getReadyParentJobs(db);
  if (jobs.length === 0) {
    db.close();
    return { success: true, synced: 0, failed: 0, results: [] };
  }

  const session = new SportlinkSession({ logger, verbose: options.verbose });
  const results = [];
  const emailParents = new Set();
  let synced = 0;
  let failed = 0;
  try {
    const page = await session.getPage();
    for (const job of jobs) {
      const desired = JSON.parse(job.desired_json);
      if (desired.mode === 'replace_email') emailParents.add(Number(job.parent_rondo_id));
      try {
        if (desired.mode === 'replace_contact') await validateParentContactJob(job, desired, options);
        const result = await writeParentJob(page, job, options);
        markParentJobSynced(db, job.id, result.slot);
        await reportParentStatus(job, 'synced', { slot: result.slot }, options);
        if (desired.mode === 'replace_contact') {
          try {
            await (options.notifyProfileChangeStatus || notifyProfileChangeStatus)(job.parent_rondo_id, desired.changes.map(c => c.syncField), 'synced', '', options);
          } catch (error) {
            logger?.verbose(`Kon profielwijzigingsstatus niet terugmelden: ${error.message}`);
          }
        }
        synced++;
        results.push({ id: job.id, success: true, slot: result.slot });
      } catch (error) {
        const blocked = markParentJobFailed(db, job, error);
        await reportParentStatus(job, blocked ? 'error' : 'pending', { message: error.message }, options);
        if (desired.mode === 'replace_contact') {
          try {
            await (options.notifyProfileChangeStatus || notifyProfileChangeStatus)(job.parent_rondo_id, desired.changes.map(c => c.syncField), blocked ? 'action_required' : 'failed', error.message, options);
          } catch (reportError) {
            logger?.verbose(`Kon profielwijzigingsstatus niet terugmelden: ${reportError.message}`);
          }
        }
        failed++;
        results.push({ id: job.id, success: false, blocked, error: error.message });
        logger?.error(`Ouder-sync voor ${job.child_knvb_id} mislukte: ${error.message}`);
      }
      await new Promise(resolve => setTimeout(resolve, 1000 + Math.random() * 1000));
    }
    const profileReporter = options.notifyProfileChangeStatus || notifyProfileChangeStatus;
    for (const parentId of emailParents) {
      if (hasUnresolvedParentJobs(db, parentId)) continue;
      try {
        await profileReporter(parentId, EMAIL_SYNC_FIELDS, 'synced', '', options);
      } catch (error) {
        logger?.verbose(`Kon profielwijzigingsstatus niet terugmelden: ${error.message}`);
      }
    }
  } finally {
    await session.close();
    db.close();
  }
  return { success: failed === 0, synced, failed, results };
}

module.exports = {
  ensureParentSyncSchema,
  getLastParentDetectionTime,
  updateLastParentDetectionTime,
  buildDesiredParent,
  findTrackedParentSource,
  fetchProfileChangesSince,
  extractEmailReplacementCandidates,
  resolveEmailReplacement,
  desiredHash,
  upsertParentJob,
  cancelMissingParentJobs,
  detectParentChanges,
  reconcileChild,
  reconcileParentEmailChanges,
  reconcileParentContactChanges,
  planParentContactReplacement,
  validateParentContactJob,
  readParentInfo,
  extractParentSlots,
  selectParentSlot,
  selectEmailReplacementSlot,
  parentValuesMatch,
  emailReplacementMatches,
  writeParentJob,
  getReadyParentJobs,
  markParentJobSynced,
  hasUnresolvedParentJobs,
  markParentJobFailed,
  runParentSlotSync,
  normalizeEmail
};
