...
async function reconcileChild(db, child, options = {}) {
  ...
  const authorizedParentIds = new Set(
    (child.parent_sync_statuses || []).map(status => Number(status.parent_id)).filter(Boolean)
  );
  const parentIds = new Set(
    (fields.relationships || [])
      .filter(relationship => relationshipSlug(relationship) === 'parent')
      .map(relatedPersonId)
      .filter(parentId => parentId && authorizedParentIds.has(parentId))
  );
  ...
  for (const parentId of parentIds) {
    ...
    try {
      const parent = await fetchPerson(parentId, options);
      const desired = buildDesiredParent(parent, child);
      const result = upsertParentJob(db, desired);
      jobRef = desired;
      if (result.changed) {
        queued++;
        await reportParentStatus(jobRef, 'pending', {}, options);
      }
    } catch (error) {
      blocked++;
      const desired = {
      ...
      await reportParentStatus(jobRef, 'error', { message: error.message }, options);
    }
  }
...
function selectParentSlot(slots, desired) {
  ...
  const matched = slots.find(slot => slot.email && slot.email === email);
  if (matched) return { slot: matched.slot, existing: true };
...
function parentValuesMatch(slot, desired) {
  return normalizeText(slot.name) === normalizeText(desired.name)
    && normalizeEmail(slot.email) === normalizeEmail(desired.email)
    && normalizeText(slot.phone) === normalizeText(desired.phone);
}
