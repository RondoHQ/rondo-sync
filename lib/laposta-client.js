...
async function upsertMember(listId, member) {
  const params = buildUpsertMemberParams(listId, member);
...
function buildUpsertMemberParams(listId, member) {
  const params = new URLSearchParams();
  params.append('list_id', listId);
  params.append('ip', '3.3.3.3');
  params.append('email', member.email);
  params.append('options[upsert]', 'true');
  // Laposta otherwise reactivates a relation who deliberately unsubscribed.
  params.append('options[suppress_reactivation]', 'true');
  params.append('options[suppress_email_notification]', 'true');
...
/**
 * Update an existing Laposta relation without changing subscription state
 * unless an explicit state is supplied.
 */
async function updateMember(listId, memberIdOrEmail, changes = {}) {
  const params = new URLSearchParams();
  params.append('list_id', listId);
  if (changes.email) params.append('email', changes.email);
  if (changes.state) params.append('state', changes.state);
  appendCustomFields(params, changes.custom_fields);

  return lapostaRequest({
    method: 'POST',
    path: `/v2/member/${encodeURIComponent(memberIdOrEmail)}`,
    body: params.toString(),
    contentType: 'application/x-www-form-urlencoded'
  });
}
