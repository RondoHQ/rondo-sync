...
// Probe URL — Sportlink redirects to its Keycloak realm under /auth/realms/
// when unauthenticated, so we can tell whether cached cookies still work.
...
class SportlinkSession {
  ...
  async _tryReuse(storageState) {
    ...
    try {
      ...
      const onAuthDomain = /\/auth\/realms\//.test(page.url());
