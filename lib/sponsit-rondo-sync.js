...
function identityMatches(candidate, person) {
  if (hasPersonalName(candidate)) {
    return personNameKey(candidate.createAcf) === personNameKey(person.fields);
  }
  return normalize(candidate.createAcf.company_name)
    && normalize(candidate.createAcf.company_name) === normalize(person.fields?.company_name);
}
...
function planRondoSponsorSync(records, rondoPeople) {
  ...
  for (const candidate of candidates) {
    ...
    if (matches.length === 1) {
      ...
      if (hasSponsorChanges(candidate.sponsorAcf, person.fields || {})) updates.push(match);
