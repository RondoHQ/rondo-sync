...
function planRondoSponsorSync(records, rondoPeople = [], rondoSponsors = []) {
  ...
  assignPrimaryPassRelations(personGroups);
...
function assignPrimaryPassRelations(personGroups) {
  for (const group of personGroups) {
    group.aliases.forEach((candidate, index) => {
      candidate.relation.is_primary_pass = index === 0 && Boolean(candidate.relation.receives_pass);
    });
  }
}
