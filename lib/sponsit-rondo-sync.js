...
const { personNameKey } = require('./person-name');
...
function planRondoSponsorSync(records, rondoPeople = [], rondoSponsors = []) {
  ...
  const people = { creates: [], updates: [], unchanged: [], deactivations: [], quarantined: [] };
  ...
  people.deactivations = planLegacySponsorRoleCleanup(rondoPeople, rondoSponsors, sponsors.archives);

  return { companies, peopleCandidates, people, sponsors, resolvedPeople };
}

/**
 * Clear legacy person-level sponsor flags once every active sponsor relation
 * for that person is being archived. The person itself is always preserved.
 */
function planLegacySponsorRoleCleanup(people, sponsors, archives) {
  const peopleById = new Map(people.map((person) => [Number(person.id), person]));
  const archiveIds = new Set(archives.map((sponsor) => Number(sponsor.id)));
  const remainingActivePersonIds = new Set();
  const archiveIdsByPerson = new Map();

  for (const sponsor of sponsors) {
    if (sponsor.status !== 'publish') continue;
    const sponsorId = Number(sponsor.id);
    for (const contact of sponsor.fields?.contacts || []) {
      const personId = Number(contact.person_id || 0);
      if (!personId) continue;
      if (!archiveIds.has(sponsorId)) {
        remainingActivePersonIds.add(personId);
        continue;
      }
      if (!archiveIdsByPerson.has(personId)) archiveIdsByPerson.set(personId, new Set());
      archiveIdsByPerson.get(personId).add(sponsorId);
    }
  }

  const deactivations = [];
  for (const [personId, personArchiveIds] of archiveIdsByPerson) {
    if (remainingActivePersonIds.has(personId)) continue;
    const person = peopleById.get(personId);
    if (!person) continue;
    const current = person.fields || {};
    const fields = {};
    if ([true, 1, '1', 'yes', 'true'].includes(current.is_sponsor)) fields.is_sponsor = false;
    if (String(current.sponsor_pass_variant || '').trim()) fields.sponsor_pass_variant = '';
    if (!Object.keys(fields).length) continue;
    deactivations.push({
      person,
      fields,
      archiveSponsorIds: [...personArchiveIds].sort((a, b) => a - b)
    });
  }
...
function assignPrimaryPassRelations(personGroups, resolvedPeople = new Map(), rondoSponsors = []) {
  ...
  const existingPrimaryKeys = new Map();
  for (const sponsor of rondoSponsors) {
    if (sponsor.status !== 'publish') continue;
    for (const contact of sponsor.fields?.contacts || []) {
      if (contact.receives_pass && contact.is_primary_pass && contact.person_id) {
        const personId = Number(contact.person_id);
        const companyId = normalize(sponsor.fields?.sponsit_contact_id);
        if (!companyId) manualPrimaryPersonIds.add(personId);
        else {
          if (!existingPrimaryKeys.has(personId)) existingPrimaryKeys.set(personId, new Set());
          existingPrimaryKeys.get(personId).add(`${companyId}|${normalize(contact.sponsit_person_id)}`);
        }
      }
    }
  }

  // Different emails can resolve through stable source IDs to the same merged person.
  const byPerson = new Map();
  for (const group of personGroups) {
    for (const candidate of group.aliases) {
      const resolution = resolvedPeople.get(candidate.sourceKey);
      const key = resolution?.personId ? `person:${resolution.personId}` : `new:${group.candidate.sourceKey}`;
      if (!byPerson.has(key)) byPerson.set(key, { personId: Number(resolution?.personId || 0), aliases: [] });
      byPerson.get(key).aliases.push(candidate);
    }
  }
  for (const { personId, aliases } of byPerson.values()) {
    const eligible = aliases.filter(candidate => candidate.relation.receives_pass);
    const existing = eligible.filter(candidate => existingPrimaryKeys.get(personId)
      ?.has(`${normalize(candidate.sponsitContactId)}|${normalize(candidate.sponsitPersonId)}`));
    const primary = manualPrimaryPersonIds.has(personId) ? null : (existing[0] || eligible[0]);
    for (const candidate of aliases) candidate.relation.is_primary_pass = candidate === primary;
  }
}
...
function changedPersonFields(desired, current) {
  const fields = {};
  const sameName = Object.hasOwn(desired, 'first_name') && Object.hasOwn(desired, 'last_name')
    && personNameKey(desired) === personNameKey(current);
  for (const [key, value] of Object.entries(desired)) {
    if (key === 'person_type') continue;
    // Keep an equivalent, already structured Rondo name instead of duplicating its infix.
    if (sameName && ['first_name', 'infix', 'last_name'].includes(key)) continue;
