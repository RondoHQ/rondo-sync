 ...
 *
 * @param {Object} acf - Rondo Club ACF fields
 * @returns {boolean} Whether this person is a local contact
 */
function isRondoClubContact(acf = {}) {
  return acf.person_type === 'contact';
}
...
function extractFieldValue(rondoClubData, field) {
  const acf = rondoClubData.fields || {};
  ...
  if (field === 'datum_vog') return acf['datum_vog'] || null;
  if (field === 'freescout_id') return acf['freescout_id'] || null;
  if (field === 'financiele_blokkade') return acf['financiele_blokkade'] || null;
...
async function fetchModifiedMembers(since, options = {}) {
  ...
  while (true) {
    ...
    // their ACF blobs over REST just to skip them in JS. Older Rondo Club
...
async function detectChanges(options = {}) {
  ...
  try {
    ...
    for (const member of modifiedMembers) {
      ...
      const datumOverlijden = acf['datum_overlijden'];
      ...
      for (const field of TRACKED_FIELDS) {
        ...
        // Skip bogus values that aren't meaningful changes
...
if (require.main === module) {
  ...
  async function selfTest() {
    ...
    // Test 1: Field extraction from fixed ACF fields
    console.log('Test 1: Field extraction from fixed ACF fields');
    const mockMember = {
      fields: {
