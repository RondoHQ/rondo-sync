...
function buildRondoSponsorCandidates(record) {
  ...
  return subjects.map((person) => {
    ...
    if (person?.id) sponsorFields.sponsit_person_id = String(person.id);
    if (companyName) sponsorFields.company_name = companyName;
    const createFields = {
      ...sponsorFields,
      person_type: 'contact',
    ...
    addIfPresent(createFields, 'gender', mapGender(sourcePerson.gender));
    addIfPresent(createFields, 'birthdate', normalizeDateOnly(sourcePerson.birthday));
    addIfPresent(createFields, 'email_2', clean(sourcePerson.email2 || contact.email2));
    addIfPresent(createFields, 'telephone_1', clean(sourcePerson.telephone1 || contact.telephone1));
    addIfPresent(createFields, 'telephone_2', clean(sourcePerson.telephone2 || contact.telephone2));
    const addresses = mapAddresses(record.addresses);
    if (addresses.length) createFields.addresses = addresses;
