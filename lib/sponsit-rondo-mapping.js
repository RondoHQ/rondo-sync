...
function buildRondoSponsorCandidates(record) {
  ...
  return subjects.map((person) => {
    ...
    if (person?.id) sponsorAcf.sponsit_person_id = String(person.id);
    ...
    addIfPresent(createAcf, 'gender', mapGender(sourcePerson.gender));
    addIfPresent(createAcf, 'birthdate', normalizeDateToYYYYMMDD(sourcePerson.birthday));
    addIfPresent(createAcf, 'email_2', clean(sourcePerson.email2 || contact.email2));
    addIfPresent(createAcf, 'telephone_1', clean(sourcePerson.telephone1 || contact.telephone1));
    addIfPresent(createAcf, 'telephone_2', clean(sourcePerson.telephone2 || contact.telephone2));
    const addresses = mapAddresses(record.addresses);
    if (addresses.length) createAcf.addresses = addresses;
