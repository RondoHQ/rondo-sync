...
function buildRondoSponsorCompanyCandidate(record) {
  ...
  const sponsorType = String(contact.type || '').toLowerCase() === 'person' ? 'person' : 'organization';
  const people = sponsorType === 'person'
    ? [buildPersonalSponsorSource(contact)]
  ...
  return {
    ...
    logo: mapSponsorLogo(contact.logo),
    title,
    status: 'publish',
    fields: {
    ...
    people: people.map((person, index) => buildPersonCandidate(person, contact, sponsitContactId, index, sponsorType))
  };
}
...
function buildPersonCandidate(person, contact, sponsitContactId, index, sponsorType = 'organization') {
  const firstName = clean(person.firstname);
  const lastName = clean(person.lastname);
  const sponsitPersonId = sponsorType === 'person'
    ? `contact:${sponsitContactId}`
    : (person.id ? Number(person.id) : null);
  const sourceKey = sponsorType === 'person'
    ? `sponsit-contact-person:${sponsitContactId}`
    : sponsitPersonId
  ...
  const fields = {
    ...
    last_name: lastName,
    email_1: normalizeEmailForRondo(person.email1)
  };
  addIfPresent(fields, 'gender', mapGender(person.gender));
  addIfPresent(fields, 'birthdate', normalizeDateOnly(person.birthday));
  addIfPresent(fields, 'email_2', normalizeEmailForRondo(person.email2));
  addIfPresent(fields, 'telephone_1', normalizePhoneForRondo(person.telephone1));
  addIfPresent(fields, 'telephone_2', normalizePhoneForRondo(person.telephone2));
  ...
  return {
    ...
    fields,
    relation: {
      contact_role: sponsorType === 'person' ? 'Sponsor' : 'Contactpersoon',
      is_primary: index === 0,
      receives_pass: true,
...
function buildPersonalSponsorSource(contact) {
  return {
    ...contact,
...
function normalizeEmailForRondo(value) {
  return clean(value).toLowerCase();
}

/** Match Rondo Club's native phone-field normalization. */
function normalizePhoneForRondo(value) {
  const phone = clean(value).replace(/[\u200B-\u200F\u202A-\u202E\uFEFF]/gu, '').trim();
  if (!phone) return '';
  if (phone.startsWith('+')) return `+${phone.slice(1).replace(/[^0-9]/g, '')}`;
  if (phone.startsWith('00')) return `+${phone.slice(2).replace(/[^0-9]/g, '')}`;
  if (phone.startsWith('0')) return `+31${phone.slice(1).replace(/[^0-9]/g, '')}`;
...
function normalizeWebsite(value) {
  ...
  if (!website) return '';
  if (/^[a-z][a-z0-9+.-]*:/i.test(website) && !/^https?:/i.test(website)) return '';
...
function mapSponsorLogo(value) {
  const sourceId = Number(value?.id);
  const filename = clean(value?.filename);
  if (!Number.isInteger(sourceId) || sourceId <= 0 || !filename) return null;
  return {
    sourceId: String(sourceId),
    filename,
    relativeUrl: `/uploads/${sourceId}/preview/${encodeURIComponent(filename)}`
  };
}
